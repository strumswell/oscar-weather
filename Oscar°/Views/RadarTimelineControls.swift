...
struct WeatherTileTimelineControls: View {
    @Bindable var tileState: WeatherTileState

    var body: some View {
        Group {
            if tileState.frameTimestamps.isEmpty {
                loadingChip
            } else {
                playerChip
            }
    ...
    private var playerChip: some View {
        VStack(spacing: 10) {
            HStack {
                Button {
                    if tileState.isPlaying { tileState.pause() } else { tileState.play() }
                    UIApplication.shared.playHapticFeedback()
                } label: {
                    Image(systemName: tileState.isPlaying ? "pause.fill" : "play.fill")
                        .font(.system(size: 15, weight: .bold))
                        .frame(width: 44, height: 44)
                        .glassEffect(in: Circle())
                }
            ...
            let frameCount = tileState.frames.count
            if frameCount > 1 {
                Slider(
                    value: Binding(
                        get: { Double(tileState.currentFrameIndex) },
                        set: { newVal in
                            let idx = Int(newVal.rounded())
                            guard idx != tileState.currentFrameIndex else { return }
                            tileState.currentFrameIndex = idx
                            UIApplication.shared.playHapticFeedback()
                        }
                    ),
                    in: 0...Double(frameCount - 1),
            ...
            HStack {
                ...
                Text(timeAt(index: twoThirdIndex))
                    ...
                    .foregroundStyle(.secondary)
                Spacer()
                Text(timeAt(index: tileState.frameTimestamps.count - 1))
                    .font(.footnote.monospacedDigit())
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var sourceBadge: some View {
        Text(tileState.currentLayer.sourceLabel)
            .font(.caption2.weight(.semibold))
            .foregroundStyle(.primary)
            .padding(.horizontal, 8)
            .padding(.vertical, 4)
    ...
    private var selectedDay: String  { dayFrom(tileState.currentFrameTimestamp) }
    private var selectedTime: String { timeFrom(tileState.currentFrameTimestamp) }

    private var oneThirdIndex: Int {
        guard tileState.frameTimestamps.count > 1 else { return 0 }
        return max(0, min(tileState.frameTimestamps.count - 1, tileState.frameTimestamps.count / 3))
    }
    private var twoThirdIndex: Int {
        guard tileState.frameTimestamps.count > 1 else { return 0 }
        return max(0, min(tileState.frameTimestamps.count - 1, (tileState.frameTimestamps.count * 2) / 3))
    }

    private func timeAt(index: Int) -> String {
        guard index >= 0, index < tileState.frameTimestamps.count else { return "--:--" }
        return timeFrom(tileState.frameTimestamps[index])
    }
