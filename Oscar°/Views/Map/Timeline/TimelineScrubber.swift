...
struct TimelineScrubber: View {
    ...
    private func trackZone(width: CGFloat, trackWidth: CGFloat, dates: [Date?],
        ...
        .accessibilityValue(
            selectedDate.map { Text("\(axisTime($0)), \(selectedIndex + 1) von \(frameCount)") }
                ?? Text("\(selectedIndex + 1) von \(frameCount)"))
    ...
    private var selectedDate: Date? {
        cachedDates.indices.contains(selectedIndex) ? cachedDates[selectedIndex] : nil
    }
